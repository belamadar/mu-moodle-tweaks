...
(function() {
    ...
    // Function to add "Add to Timeline" buttons to course content with .activityitem class
    function addAddToTimelineButtons() {
        // Select all activity items
        const contentItems = document.querySelectorAll('.activityitem');

        contentItems.forEach(item => {
            // Skip if the button already exists
            if (item.querySelector('.add-to-timeline-button')) {
                return;
            }

            // Create the "Add to Timeline" button
            const addButton = document.createElement('button');
            addButton.textContent = 'Add to Timeline';
            addButton.className = 'btn btn-primary add-to-timeline-button';
            addButton.style.marginLeft = '10px';

            // Add click event listener to save the item to timeline
            addButton.addEventListener('click', () => {
                const itemTitle = item.textContent.trim(); // Use the text of the .activityitem itself as the title
                const deadline = prompt("Enter deadline for this item (e.g. 2024-11-20):"); // Prompt for deadline

                if (itemTitle && deadline) {
                    let timelineItems = JSON.parse(localStorage.getItem('customTimelineItems')) || [];
                    timelineItems.push({ title: itemTitle, deadline: deadline });
                    localStorage.setItem('customTimelineItems', JSON.stringify(timelineItems));

                    alert('Item added to your timeline!');
                }
            });

            // Append the button to the content item
            item.appendChild(addButton);
        });
    }

    // Function to add custom items to the timeline section
    function updateTimeline() {
        const timelineSection = document.querySelector('#yui_3_17_2_1_1729166366693_336');

        if (timelineSection) {
            let timelineItems = JSON.parse(localStorage.getItem('customTimelineItems')) || [];

            // Clear previous custom timeline items if present
            const existingCustomItems = timelineSection.querySelectorAll('.custom-timeline-item');
            existingCustomItems.forEach(item => item.remove());

            // Add each custom timeline item
            timelineItems.forEach(timelineItem => {
                const customItem = document.createElement('div');
                customItem.className = 'custom-timeline-item p-2 mb-2';
                customItem.style.border = '1px solid #ccc';
                customItem.style.backgroundColor = '#f9f9f9';
                customItem.textContent = `${timelineItem.title} - Due: ${timelineItem.deadline}`;

                timelineSection.appendChild(customItem);
            });
        }
    }

    // Run the functions after page load
    window.addEventListener('load', () => {
        addAddToTimelineButtons();
        updateTimeline();

        // Set an interval to ensure buttons and custom items are added after dynamic loading
        setInterval(() => {
            addAddToTimelineButtons();
            updateTimeline();
        }, 2000);
